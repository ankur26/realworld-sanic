...
def test_create_article(app):
    ...
    with client:
        ...
        _, response = client.post("/api/users/login", json=user_for_profile_1)
        ...
        _, response = client.post("/api/users/login", json=user_for_profile_2)
        assert response.status == 200
        token = response.json.get("user").get("token")
        # Test an invalid response
        _, response = client.post(
        ...
        assert response.json.get("article").get("title") == article_2.get(
            "article"
        ).get("title")
        assert (
            response.json.get("article").get("author").get("username")
            == user_for_profile_2_registration["user"]["username"]
        )
...
def test_get_article_anonymous(app):
    ...
    with client:
        ...
        assert response.status == 200
        assert (
            response.json.get("article").get("title") == article_1["article"]["title"]
        )
...
def test_get_article_non_anonymous(app):
    ...
    with client:
        ...
        _, response = client.get(
            "api/articles/{}".format(slug1), headers={"Authorization": f"Token {token}"}
        )
        assert response.status == 200
        assert (
            response.json.get("article").get("title") == article_1["article"]["title"]
...
def test_favorites_and_favorited_count(app):
    ...
    with client:
        ...
        _, response = client.post(
            "api/articles/{}/favorite".format(slug1),
            headers={"Authorization": f"Token {token}"},
        )
        assert response.status == 200
...
def test_update_article(app):
    ...
    with client:
        ...
        _, response = client.delete(
            "api/articles/{}".format(slug2), headers={"Authorization": f"Token {token}"}
        )
        assert response.status == 200
        ...
        assert (
            response.status == 403
        )  # We should not be allowed to delete someone elses slug
...
def test_update_article(app):
    ...
    with client:
        ...
        _, response = client.put(
            "/api/articles/{}".format(slug2),
            headers={"Authorization": f"Token {token}"},
            json=update_article,
        )
        ...
        assert (
            response.json.get("article").get("title")
            == update_article["article"]["title"]
        )
