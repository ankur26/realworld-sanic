...
from sanic import SanicException, Unauthorized
...
def validate_request_body_exists(func):
    ...
    async def wrapper(request, *args, **kwargs):
        ...
        if not request.body:
            raise SanicException("body - Cannot be empty", 422)
...
def validate_request_object_exists_in_body(key):
    ...
    def decorator(func):
        ...
        async def wrapper(request, *args, **kwargs):
            logger.info(
                f"validate_request_object_exists_in_body: checking body for {key} object"
            )
            if not request.json.get(key, None):
                raise SanicException(f"body- {key} cannot be empty", 422)
...
def validate_authorization_token_exists(allow_anonymous=False):
    def dec(func):
        ...
        async def wrapper(request, *args, **kwargs):
            ...
            if auth_header:
                logger.info(f"found auth {auth_header}")
                ...
                if allow_anonymous:
                    ...
                    raise Unauthorized("headers- Missing Bearer token")
