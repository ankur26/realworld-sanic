...
async def check_token_and_return_status(token:str):
    ...
    try:
        claims = decode(jwt=token,key=key,algorithms=["HS256"],audience="user",issuer="conduit-sanic")
        ...
        # The recommmended solution is to never go with somet with
        if user_from_id.id == user_from_username.id and user_from_id.username == user_from_username.username:
